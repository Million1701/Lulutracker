...
const PetForm: React.FC<PetFormProps> = ({
  ...
  const handleExtraPhotosChange = (e: ChangeEvent<HTMLInputElement>): void => {
    ...
    filesToAdd.forEach((file) => {
      if (file.size > 5 * 1024 * 1024) {
        setErrors((prev) => ({
          ...prev,
          extra_photos: 'Cada imagen no debe superar 5MB',
        }));
        return;
      }
      ...
      reader.onloadend = () => {
        ...
        setPreviews((prev) => ({
          ...prev,
          extra_photos: [...prev.extra_photos, imageUrl],
        }));
        setFormData((prev) => ({
          ...prev,
          extra_photos: [...(prev.extra_photos || []), imageUrl],
  ...
  const removeExtraPhoto = (index: number): void => {
    setPreviews((prev) => ({
      ...prev,
  ...
  return (
    <div className="max-w-2xl mx-auto w-full md-w-screen h-full sm:h-fit overflow-auto">
      <Card className="p-8 rounded-none sm:rounded-lg">
